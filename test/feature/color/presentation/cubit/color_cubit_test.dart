...
void main() 
{
  ...
  group('Color cubit tests:', () 
  {
    ...
    blocTest('Успешное сохранение цвета', build: ()
    {
      ...
      final GetColorUseCase getColorUseCase = MockGetColorUseCase();
      final ColorEntity colorEntity = ColorEntity(color: Colors.black.value.toString());
      final ColorEntity newColorEntity = ColorEntity(color: const Color.fromARGB(210, 22, 118, 207).value.toString());
    ...
    act: (cubit) => cubit.setColor(const Color.fromARGB(210, 22, 118, 207)),
    expect: () => 
    [
      const ColorCurrentState(color: Colors.black),
      const ColorCurrentState(color: Color.fromARGB(210, 22, 118, 207)),
    ],
    );
  }
